    ...
    * ---------------------------------------------------------------- */
    init(eoj_list) {
        return new Promise((resolve, reject) => {
            ...
            //     });
            // }
            // EOJ リスト情報に EOJ が 1 つもなければ蓄電池を追加
            if (new_eoj_list.length === 0) {
                let desc = this._mDeviceDescription.getEoj('027D');
                new_eoj_list.push({
                    eoj: '027D01',
                    epc: Object.keys(desc['elProperties']),
                    release: this._mDeviceDescription.getRelease()
    ...
    async _initNodeProfileInstance(devobj) {
        // サポートする EOJ のリストとノードクラスのリスト
        const eoj_list = [];
        const class_hash = {};

        for (let eoj of Object.keys(this._device_objects)) {
            eoj_list.push(eoj);
            let class_code = eoj.substring(0, 4);
        ...
        const d6 = [];
        const d6_eoj_num = eoj_num - 1;
        if (d6_eoj_num > 255) {
            d6_eoj_num = 255;
        }
        const d6_eoj_list_full = JSON.parse(JSON.stringify(eoj_list));
        let d6_eoj_list = [];
        for (let eoj of d6_eoj_list_full) {
            if (!/^0EF0/.test(eoj)) {
    ...
    * ---------------------------------------------------------------- */
    sendPacket(address, packet) {
        return new Promise((resolve, reject) => {
            // 本デバイスがパワーオフの状態ならエラー
            if (this._power_status === false) {
                reject(new Error('The power status is off. Turn on this emulator.'));
            }
    ...
    send(address, buf) {
        return new Promise((resolve, reject) => {
            ...
            this._packet_sender.send(address, buf).then((dest_addr) => {
                ...
                // パケットエラーログに出力
                if (this._packet_logger) {
                    const dest_addr = address || this._ip_address_utils.getMulticastAddress();
        ...
        if ((esv === '72' && epcs['D6']) || (esv === '73' && epcs['D5'])) {
            ...
            this._getRemoteDeviceId(address).then((id) => {
                ...
                try {
                    ...
                    // 各EOJに対してIPアドレスを保存
                    rdev['eojList'].forEach(eojObj => {
                        if (eojObj && eojObj.eoj) {
                            mapping[eojObj.eoj] = address;
                        }
                    });
                    mFs.writeFileSync(mappingPath, JSON.stringify(mapping, null, 2), 'utf8');
                } catch (err) {
                    console.error('Failed to save EOJ-IP mapping:', err);
                }
