    ...
    * ---------------------------------------------------------------- */
    request(req) {
        ...
        * ---------------------------------------------- */
        if (k === '/api/system/lang get') {
            ...
            * /api/drDevices
            * ---------------------------------------------- */
        } else if (/^\/api\/drDevices\/([0-9A-Fa-f]{6})\/properties\/([0-9A-Fa-f]{2}) get/.test(k)) {
            // リモートデバイスの EPC データ (EDT) の個別取得（DR用）
            const drDevicesMatch = req['path'].match(/^\/api\/drDevices\/([0-9A-Fa-f]{6})\/properties\/([0-9A-Fa-f]{2})$/);
    ...
    _drDevicesPropertyGet(req, device_id, property_name) {
        return new Promise((resolve, reject) => {
            ...
            // 既存のEchonet Liteプロパティ取得関数を利用
            this._device.getRemoteDeviceEpcData(ip_address, device_id, property_name)
                .then(result => {
    ...
    _drDevicesPropertyPut(req, device_id, property_name) {
        return new Promise((resolve, reject) => {
            ...
            this._device.setRemoteDeviceEpcData(ip_address, device_id, property_name, edt)
                .then(result => {
                    if (result.result === 0) {
