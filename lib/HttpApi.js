...
class HttpApi {
    ...
    * ---------------------------------------------------------------- */
    request(req) {
        ...
        * ---------------------------------------------- */
        if (k === '/api/system/lang get') {
            ...
            * /api/drDevices
            * ---------------------------------------------- */
        } else if (/^\/api\/drDevices\/([0-9A-Fa-f]{6})\/properties\/([^\/]+) get/.test(k)) {
            // リモートデバイスの EPC データ (EDT) の個別取得（DR用）
            const drDevicesMatch = req['path'].match(/^\/api\/drDevices\/([0-9A-Fa-f]{6})\/properties\/([^\/]+)$/);
            return this._drDevicesPropertyGet(req, drDevicesMatch[1], drDevicesMatch[2]);
        } else if (/^\/api\/drDevices\/([0-9A-Fa-f]{6})\/properties\/([^\/]+) put$/.test(k)) {
            // リモートデバイスの EPC データ (EDT) の個別設定（DR用）
            const drDevicesMatch = req['path'].match(/^\/api\/drDevices\/([0-9A-Fa-f]{6})\/properties\/([^\/]+)$/);
            return this._drDevicesPropertyPut(req, drDevicesMatch[1], drDevicesMatch[2]);
    ...
    // 新API: /api/drDevices/:device_id/properties/:property_name
    _drDevicesPropertyGet(req, device_id, property_name) {
        return new Promise((resolve, reject) => {
    ...
    // PUT: /api/drDevices/{device_id}/properties/{property_name}
    _drDevicesPropertyPut(req, device_id, property_name) {
        return new Promise((resolve, reject) => {
            const mFs = require('fs');
            const mPath = require('path');
            // EOJ-IPマッピングファイルのパス
