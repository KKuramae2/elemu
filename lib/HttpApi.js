...
// --- shortName <-> EPC マッピングユーティリティ ---
const fs = require('fs');
const path = require('path');

function getDeviceTypeFromEoj(eoj) {
    if (typeof eoj === 'string' && eoj.length >= 5) {
        // 例: '027D01' → '027D'
        return eoj.slice(0, 4).toUpperCase();
    }
    return null;
}

function getEpcShortNameMap(eoj) {
    const deviceType = getDeviceTypeFromEoj(eoj);
    const devicePath = path.resolve(__dirname, `../conf/mraData/devices/0x${deviceType}.json`);
    const superPath = path.resolve(__dirname, '../conf/mraData/superClass/0x0000.json');
    let map = {};
    // 機種別
    if (fs.existsSync(devicePath)) {
        const obj = JSON.parse(fs.readFileSync(devicePath, 'utf8'));
        const arr = Array.isArray(obj) ? obj : (Array.isArray(obj.elProperties) ? obj.elProperties : []);
        arr.forEach(prop => {
            // epc/shortName両方対応
            if ((prop.EPC || prop.epc) && prop.shortName) {
                const epcVal = (prop.EPC || prop.epc).replace(/^0x/i, '').toUpperCase();
                map[prop.shortName] = epcVal;
                map[epcVal] = prop.shortName;
            }
        });
    }
    // 共通
    if (fs.existsSync(superPath)) {
        const obj = JSON.parse(fs.readFileSync(superPath, 'utf8'));
        const arr = Array.isArray(obj) ? obj : (Array.isArray(obj.elProperties) ? obj.elProperties : []);
        arr.forEach(prop => {
            if ((prop.EPC || prop.epc) && prop.shortName && !map[prop.shortName]) {
                const epcVal = (prop.EPC || prop.epc).replace(/^0x/i, '').toUpperCase();
                map[prop.shortName] = epcVal;
                map[epcVal] = prop.shortName;
            }
...
function resolveEpcFromPropertyName(eoj, property_name) {
    const map = getEpcShortNameMap(eoj);
    // property_nameがEPC値（16進2桁）ならそのまま
    if (/^[0-9A-Fa-f]{2}$/.test(property_name)) return property_name.toUpperCase();
    // shortNameならマッピング
    if (map[property_name]) return map[property_name];
    return null;
}
...
class HttpApi {
    ...
    * ---------------------------------------------------------------- */
        ...
        * ---------------------------------------------- */
        if (k === '/api/system/lang get') {
            ...
            * ---------------------------------------------- */
            ...
            /* ------------------------------------------------
            * WebAPI経由ELデバイス操作用追加API（IPアドレス指定不要）
            * /api/elDevices
            * ---------------------------------------------- */
        } else if (/^\/api\/elDevices\/([0-9A-Fa-f]{6})\/properties\/([^\/]+) get/.test(k)) {
            // リモートデバイスの EPC データ (EDT) の個別取得（DR用）
            const elDevicesMatch = req['path'].match(/^\/api\/elDevices\/([0-9A-Fa-f]{6})\/properties\/([^\/]+)$/);
            return this._elDevicesPropertyGet(req, elDevicesMatch[1], elDevicesMatch[2]);
        } else if (/^\/api\/elDevices\/([0-9A-Fa-f]{6})\/properties\/([^\/]+) put$/.test(k)) {
            // リモートデバイスの EPC データ (EDT) の個別設定（DR用）
            const elDevicesMatch = req['path'].match(/^\/api\/elDevices\/([0-9A-Fa-f]{6})\/properties\/([^\/]+)$/);
            return this._elDevicesPropertyPut(req, elDevicesMatch[1], elDevicesMatch[2]);
    ...
    // 新API: /api/elDevices/:device_id/properties/:property_name
    _elDevicesPropertyGet(req, device_id, property_name) {
        return new Promise((resolve, reject) => {
            const mFs = require('fs');
            const mPath = require('path');
            // EOJ-IPマッピングファイルのパス
            const mappingPath = mPath.resolve(__dirname, '../data/remote__device.json');
            let mapping = {};
            if (mFs.existsSync(mappingPath)) {
                mapping = JSON.parse(mFs.readFileSync(mappingPath, 'utf8'));
            }
            const ip_address = mapping[device_id];
            if (!ip_address) {
                this._setReqToErrorRes(req, 1, 404, 'Device not found for EOJ: ' + device_id);
                resolve(req);
                return;
            }
            // property_nameがshortNameの場合はEPC値に変換
            const epc = resolveEpcFromPropertyName(device_id, property_name);
            if (!epc) {
                this._setReqToErrorRes(req, 1, 400, 'Invalid property_name: ' + property_name);
                resolve(req);
                return;
            }
            this._device.getRemoteDeviceEpcData(ip_address, device_id, epc)
                .then(result => {
                    if (result.result === 0 && result.elProperty) {
                        this._setReqToSuccessRes(req, { value: result.elProperty });
                    } else {
                        this._setReqToErrorRes(req, 1, 500, result.message || 'Failed to get property');
                    }
                    resolve(req);
                })
                .catch(err => {
                    this._setReqToErrorRes(req, 1, 500, err.message || 'Internal error');
                    resolve(req);
                });
        });
    }

    // PUT: /api/elDevices/{device_id}/properties/{property_name}
    _elDevicesPropertyPut(req, device_id, property_name) {
        return new Promise((resolve, reject) => {
            const mFs = require('fs');
            const mPath = require('path');
            // EOJ-IPマッピングファイルのパス
            ...
            // パラメータチェック
            const p = req['params'];
            if (!p || typeof p !== 'object' || !('edt' in p)) {
                this._setReqToErrorRes(req, 1, 400, 'Parameter edt is required.');
                resolve(req);
                return;
            }
            const edt = p['edt'];

            // 既存のEchonet Liteプロパティ設定関数を利用
            this._device.setRemoteDeviceEpcData(ip_address, device_id, epc, edt)
                .then(result => {
                    if (result.result === 0) {
                        this._setReqToSuccessRes(req, { property: result.property });
                    } else {
                        this._setReqToErrorRes(req, 1, result.code || 500, result.message || 'Failed to set property');
                    }
                    resolve(req);
                })
                .catch(err => {
